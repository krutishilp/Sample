...
public class TicketRepositoryJDBCImpl implements TicketRepository {
	...
	public Ticket[] displayAllTickets(int userId) {
		...
		try {
			...
			while (rs.next()) {
				...
				list.add(new Ticket(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5),
		...
		try {
			...
			if (rs.next()) {
				...
				try {
					...
					pstmt.setLong(2, passengerContact);
					...
					if (ch > 0) {
						...
						if (rs.next()) {
							...
							ticket = new Ticket(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4),
									rs.getString(5), rs.getInt(6), payflag, cancelflag, rs.getInt(9));
							return ticket;
	...
	public Passenger getPassengerDetails(int ticketId) throws TicketNotFoundException {
		...
		try {
			pstmt = con.prepareStatement(
					"select passengerId from ticket where ticketId=?");
			pstmt.setInt(1, ticketId);
			rs = pstmt.executeQuery();
			if(rs.next()) {
				...
				rs=pstmt.executeQuery();
				if(rs.next()) {
					passenger=new Passenger(rs.getInt(1),rs.getString(2),rs.getLong(3),rs.getString(4));
					return passenger;
				}
			}else {
				throw new TicketNotFoundException("Ticket Not Found");
